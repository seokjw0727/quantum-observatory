...
def build(data_dir,output):
    ...
    for r in full:
        ...
        small={k:v for k,v in r.items() if k not in {'abstract','events','provenance','version'}}
    ...
    editorial_index=[{key:article[key] for key in ['slug','type','title','description','summary','author','published_at','updated_at']} | {'path':article_path(article),'topics':article.get('topics',[]),'paper_ids':article.get('paper_ids',[])} for article in articles]
    ...
    write_page(stage,'topics/error-correction',render_error_correction_topic(site_config,articles))
    ...
    public_paths=['/','/research/','/archive/','/methodology/','/analysis/','/guides/','/about/','/contact/','/editorial-policy/','/privacy/','/terms/','/topics/error-correction/']+[article_path(article) for article in articles]
